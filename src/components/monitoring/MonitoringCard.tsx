...
/** Simple inline SVG sparkline — draws a minified line chart without Recharts overhead. */
...
function formatWIB(date: Date | string | null): string {
  if (!date) return "-";
...
export default function MonitoringCard({ device }: Props) {
  ...
  // Sparkline data: last N readings for temperature
  ...
  return (
    <div
      className={`rounded-xl border p-4 sm:p-5 shadow-sm transition-all hover:shadow-md ${
        hasAlert
      ...
      <div className="mt-4 sm:mt-5 grid grid-cols-2 gap-3 sm:gap-4">
        ...
        >
          ...
          >
            Temperature
            {tempTrend === "up" && (
              <span className="ml-1.5 inline-block text-[10px]">↑</span>
            )}
            {tempTrend === "down" && (
              <span className="ml-1.5 inline-block text-[10px]">↓</span>
            )}
