  ...
  const getContextInfo = () => {
    if (activeTab === "daily-trend") {
      return {
        ...
        description: "Rata-rata, minimum & maksimum suhu per jam hari ini (WIB)",
        unit: "°C",
    ...
    if (activeTab === "comparison-temp") {
      return {
        ...
        description: "Perbandingan suhu realtime antar lokasi (PDB, UPS, Battery)",
        unit: "°C",
    ...
    <div className="rounded-xl border bg-white dark:bg-slate-800 shadow-sm overflow-hidden">
      ...
      <div className="p-3 sm:p-4 md:p-5 pb-3 sm:pb-4 border-b border-slate-200 dark:border-slate-700">
        <div className="flex flex-col gap-3 mb-3 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* Live Indicator */}
          <div className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 w-fit shrink-0">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <div className="relative px-3 sm:px-4 md:px-5 pt-3 sm:pt-4">
          <TabsList className="w-full justify-start overflow-x-auto bg-slate-100 dark:bg-slate-900 p-1 rounded-lg gap-1 scrollbar-hide flex-nowrap">
            <TabsTrigger value="daily-trend" className="shrink-0 whitespace-nowrap data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 shadow-sm px-3 py-1.5 text-xs sm:text-sm">
              📈 Harian
            </TabsTrigger>
            <TabsTrigger value="comparison-temp" className="shrink-0 whitespace-nowrap data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 shadow-sm px-3 py-1.5 text-xs sm:text-sm">
              🌡️ Suhu
            </TabsTrigger>
            <TabsTrigger value="comparison-hum" className="shrink-0 whitespace-nowrap data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 shadow-sm px-3 py-1.5 text-xs sm:text-sm">
              💧 Humidity
            </TabsTrigger>
            {devices.map((device) => (
      ...
      <div className="p-3 sm:p-4 md:p-5">
        ...
        <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-start gap-2 sm:gap-3 flex-1 min-w-0">
            <div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl bg-slate-100 dark:bg-slate-800 mt-0.5 sm:mt-1 shadow-sm shrink-0">
              {context.icon}
            </div>
        ...
        >
          {isLoading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-4">
              ...
              <p className="mt-3 text-xs sm:text-sm font-semibold text-center">Memuat data chart...</p>
            </div>
          ) : (
            <>
              ...
              {!activeTab.startsWith("comparison") && activeTab !== "daily-trend" && singleDeviceData.length === 0 && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg sm:rounded-xl p-4">
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-center">Tidak ada data telemetry</p>
                  <p className="text-[10px] sm:text-xs mt-1 text-center">Device mungkin offline atau belum melaporkan data</p>
                </div>
              )}
            </>
          )}
